        ...
        _.each(Object.keys(groupedRelations), (key) => {
            ...
            if (reference.config.type === 'manyToMany') {
                ...
                if (_.every(statements.map(s => s.operator), isCompOp)) {
                    ...
                    const compOp = negateGroup
                        ? 'whereNot'
                        : 'where';

                    // CASE: WHERE resource.id (IN | NOT IN) (SELECT ...)
                    const parentTalbeKey = `${this.tableName}.id`;
                    const joinFilterStatements = groupedRelations[key].joinFilterStatements;

                    let joinValue = reference.config.tableName;
                    let innerJoinOn = `${reference.config.tableName}.${reference.config.joinFrom}`;

                    // CASE: you can define a name for the join table
                    if (reference.config.tableNameAs) {
                        joinValue = `${reference.config.tableName} as ${reference.config.tableNameAs}`;
                        innerJoinOn = `${reference.config.tableNameAs}.id`;
                    }

                    qb.where(function () {
                        // TODO: reproduce `null` case and maybe remove it alltogether
                        if (negateGroup && reference.value !== null) {
                            this.whereNot((`${joinValue}.${reference.column}`, reference.value));
                            // qb.orWhereNull(`${joinValue}.${reference.column}`);
                        } else {
                            this.where((`${joinValue}.${reference.column}`, reference.value));
                        }
                    })
                        .join(joinValue, function () {
                            this.on(innerJoinOn, '=', parentTalbeKey);

                            // TODO: check this in 1:1 relation
                            // CASE: when applying AND con junction and having multiple groups the filter
                            //       related to joining table has to be applied within each group
                            _.each(joinFilterStatements, (joinFilter) => {
                                this.andOn(`${joinFilter.joinTable}.${joinFilter.column}`, compOps[joinFilter.operator], joinFilter.value);
                            ...
                            if (debugExtended.enabled) {
                                debug(`(buildRelationQuery) innerQB sql-pre: ${qb.toSQL().sql}`);
                            }
                            ...
                            if (debugExtended.enabled) {
                                debug(`(buildRelationQuery) innerQB sql-post: ${this.toSQL().sql}`);
                            }

                            return qb;
                        });
                    console.log(qb.toSQL().sql);
                } else {
                    debug(`one of ${key} group statements contains unknown operator`);
                }
            }
