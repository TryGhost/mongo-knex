        ...
        _.each(Object.keys(groupedRelations), (key) => {
            ...
            if (reference.config.type === 'manyToMany') {
                ...
                if (_.every(statements.map(s => s.operator), isCompOp)) {
                    ...
                    if (reference.whereType === 'whereNotNull') {
                        qb._joins = {type: 'leftJoin', joinValue, parentTalbeKey, innerJoinOn};
                        qb[reference.whereType](`${joinValue}.${reference.column}`);
                    } else {
                        qb[reference.whereType](`${joinValue}.${reference.column}`, compOps[reference.operator], reference.value);
