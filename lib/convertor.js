...
const regexEnabledJSONStringify = (json) => {
...
const getValueFromRegex = (regex) => {
    return regex.toString().replace(/\//g, '%').replace('i', '').toLowerCase();
}
...
class MongoToKnex {
    ...
    buildRelationQuery(qb, relations, mode) {
        ...
        if (debugExtended.enabled) {
            debugExtended(`(buildRelationQuery) ${regexEnabledJSONStringify(relations)}`);
        }

        const groupedRelations = this.groupRelationStatements(relations, mode);

        if (debugExtended.enabled) {
            debugExtended(`(buildRelationQuery) grouped: ${regexEnabledJSONStringify(groupedRelations)}`);
        }
    ...
    buildComparison(qb, mode, statement, op, value, group) {
        ...
        if (op === '$regex' || op === '$not') {
            whereType += 'Raw';
            value = getValueFromRegex(processedStatement.value)
            debug(`(buildComparison) whereType: ${whereType}, statement: ${statement}, op: ${op}, comp: ${comp}, value: ${value} (REGEX)`);
            qb[whereType](`lower(??) ${comp} ?`, [processedStatement.column, value]);
        } else {
    ...
    buildWhereClause(qb, mode, statement, sub, group) {
        ...
        if (debugExtended.enabled) {
            debugExtended(`(buildWhereClause) ${regexEnabledJSONStringify(sub)}`);
        }
    ...
    buildWhereGroup(qb, parentMode, mode, sub) {
        ...
        if (debugExtended.enabled) {
            debugExtended(`(buildWhereGroup) ${regexEnabledJSONStringify(sub)}`);
        }
    ...
    buildQuery(qb, mode, sub, group) {
        ...
        if (debugExtended.enabled) {
            debugExtended(`(buildQuery) ${regexEnabledJSONStringify(sub)}`);
        }
    ...
    processJSON(qb, mongoJSON) {
        ...
        if (debugExtended.enabled) {
            debugExtended(`(processJSON) ${regexEnabledJSONStringify(mongoJSON)}`);
        }
